...
Nella tabella \ref{tab:esempio} riportiamo una breve descrizione generale del dataset,  nella figura ![Figure 1: Correlation matrix](immagine/aqi_ist.png) l'istogramma della variabile AQI e nella figura  la matrice di correlazione delle variabili.

\begin{table}[h]
    \centering
    \begin{tabular}{l r| l}  % 3 colonne: sinistra, destra, sinistra
        \toprule
        Statistiche del dataset & \quad & Tipo delle variabili \\
        \midrule
        Numero di variabili     & 16 \quad     & Categoriche: 0 \\
        Numero di osservazioni  & 1096 \quad  & DateTime: 1 \\
        Valori mancanti         & $\sim 15\%$\quad & Numeriche: 15 \\
        Righe duplicate         & 0      & \quad \\
        \bottomrule
    \end{tabular}
    \caption{Descrizione del dataset}
    \label{tab:esempio}
\end{table}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{aqi_ist.png}
    \caption{Istogramma di AQI}
    \label{fig:AQI_ist}
\end{figure}

\begin{figure}[H]
    \includegraphics[width=0.9\linewidth]{matrice di correlazione.png}
...
\subsection*{Identificazione e validazione degli Outlier}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{schema_lavoro_.png}
    \caption{Schema del workflow adottato per l’identificazione degli outlier e la predizione dell’AQI}
    \label{fig:workflow}
\end{figure}
...
Alla luce di tali considerazioni, si è scelto di applicare la trasformazione di Yeo–Johnson \cite{gee25_1} alle variabili AQI, $PM_{2.5}$ e $PM_{10}$,  al fine di rendere le loro distribuzioni marginali — e di conseguenza la distribuzione congiunta — più prossime alla gaussianità (Figura \ref{fig:AQI transf}).

\noindent\textbf{Risultato teorico.}\cite{mahadist}
Se $\mathbf{X} \sim \mathcal{N}_n(\mu,\Sigma)$ $\Rightarrow
$ la distanza di Mahalanobis definita da $ d^2(\textbf{x}) = (\textbf{x}-\mu)^\top \Sigma^{-1}(\textbf{x}-\mu)$ $\sim \chi^2_n $.
Di conseguenza, per determinare la soglia $q_p $ di confidenza a livello $\alpha$ tale che $\mathbb{P}\left(d^2(\textbf{x}) < q_p\right) = \alpha$, utilizziamo  $ q_p = \chi^2_{1-\alpha,n}$, dove $n$ è il numero di variabili considerate.
...
\begin{figure}[H]
    ...
    \caption{Outlier finali per AQI}
    \label{fig:out_plot}
\end{figure}
...
\cite{hastie2009elements}\cite{james2013introduction}
...
\begin{table}[h]
    ...
    \begin{tabular}{l| l |l}  
        ...
        \midrule
        $\sum_i^n  (Y_i-\hat{Y_i})^2$  & $\sum_i^n  (Y_i-\hat{Y_i})^2 + \lambda \sum_j \beta_j^2$    &   $\sum_i^n  (Y_i-\hat{Y_i})^2 + \lambda \sum_j |\beta_j|$\\
    \bottomrule
    \end{tabular}
    \caption{Funzioni obiettivo per i diversi tipi di regressione lineare}
    \label{tab:obj_func}
\end{table}
...
\begin{itemize}
    \item il \emph{training set}, utilizzato in prima battuta per effettuare la validazione incrociata, attraverso la quale la procedura adoperata per ottimizzare l'iperparametro $\lambda$ stabilisce la configurazione ottimale, e successivamente per ottenere la stima dei coefficienti del modello lineare una volta fissato l'iperparametro scelto.
    \item il \emph{test set} mantenuto separato durante la fase di addestramento e impiegato unicamente per la valutazione delle prestazioni predittive del modello lineare. 
    \end{itemize}
...
\subsubsection*{Modelli ensamble}
...
\subsubsection*{Regessore K-Nearest Neighbors}
...
\subsection*{Metodi per l'ottimizzazione degli iper-parametri}
...
\begin{itemize}
    \item riduce l'impegno e il tempo che l'uomo deve dedicare al ML
    \item incide in maniera determinante sulla performance degli algoritmi garantendone il miglioramento
    \item migliora la riproducibilità e l'imparzialità degli studi scientifici
\end{itemize}

Matematicamente, l'HPO rappresenta la procedura attraverso cui selzionare il valore di un certo iperparametro $\lambda$, in modo da minimizzare l'errore generalizzato definito come $\mathbb{E}_{X\sim \mathrm{D}}\left[\mathcal{L}(X,\mathcal{A}_{\lambda}(X^{(train)})) \right]$. In altri termini, $\lambda^{*}$, ovvero la miglior scelta possibile dell'iperparametro, deve soddisfare:
...
$\mathcal{L}$, si adotta il protocollo della \textbf{cross-validation}, che consiste nel suddividere il dataset di training in più sottoinsiemi casuali e nel valutare le prestazioni del modello attraverso ripetute fasi di addestramento e validazione. Questo approccio consente di ottenere una stima più robusta e affidabile della performance del modello, riducendo la dipendenza dai risultati ottenuti su un singolo campione dei dati.\cite{hutter2019automated}

\subsubsection*{Grid e Randomized Search}
...
\subsubsection*{Ricerca Bayesiana}
...
Dal momento che la ricerca bayesiana applicata all'HPO indirizza l'esplorazione verso le regioni più promettenti dello spazio degli iperparametri, può risultare molto più efficiente rispetto a Grid e Randomized Search.\cite{snoek2012practicalbayesianoptimizationmachine}\cite{watanabe2025treestructuredparzenestimatorunderstanding}
\vspace{1cm}



Nel presente lavoro sono state implementate tutte le tecniche di HPO descritte. In particolare, per quanto riguarda l’approccio di ricerca bayesiana, si è fatto ricorso ad  Optuna \cite{ozaki2025optunahub}, un potente  framework di ottimizzazione degli iperparametri in Python progettato per automatizzare e semplificare il processo di ottimizzazione.

Gli iperparametri oggetto di ottimizzazione includono :
\begin{itemize}
    \item il parametro di regolarizzazione $\lambda$ nei modelli di Ridge e Lasso Regression, che controlla l’intensità della penalizzazione applicata ai coefficienti
    \item la profondità massima degli alberi \texttt{max\_depth} e il numero di stimatori , \texttt{n\_estimators} nei modelli Random Forest ed Extra Trees Regressor che influenzano rispettivamente la complessità del modello e la sua capacità di generalizzazione
    \item la profondità massima degli alberi nel Gradient Boosting Regressor, parametro che regola il compromesso tra accuratezza e rischio di overfitting.
    \item il parametro $K$ del Nearest-Neaghboor Regressor che stabilisce il numero di punti più vicini da considerare
\end{itemize}
\subsection*{Metriche per la valutazione della performance}
...
dove $\overline{Y}=\frac{\sum_{i=1}^n Y_i}{n}$
\subsubsection*{MSE}
...
\section*{Confronto dei risulati e conclusioni}
\subsection*{Analisi comparativa dei modelli prima e dopo l'eliminazione degli outlier}
...
In particolare, i modelli ensemble — Random Forest Regressor ed Extra Trees Regressor — sono quelli che hanno beneficiato maggiormente della rimozione degli outlier (Figura \ref{fig:y} e Figura \ref{fig:r^2}), con valori di $R^2$ che passano da circa $0.8$ o leggermente inferiori (Extra Trees Regressor) a valori prossimi a $0.9$. Sebbene non della stessa entità, anche il modello K-Nearest Neighbors mostra un miglioramento apprezzabile, evidenziando  come il calcolo delle distanze, operazione cruciale per mettere a punto le predizioni, sia fortemente influenzato dai valori anomali.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\linewidth]{y_comp.png}
    \caption{Confronto tra gli $Y_i$ e gli $\hat{Y_i}$ per il modello RFR dopo la rimozione degli outlier}
    \label{fig:y}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.99\linewidth]{bar_plot.png}
    \caption{Confronto della performance dei modelli prima e dopo la rimozione degli outlier}
    \label{fig:r^2}
\end{figure}
